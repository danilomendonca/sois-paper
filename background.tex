...
Bring background information about Wi-Fi zones and D2D communication technology, including Wi-Fi direct and near-future D2D in 5G

\begin{figure*}[t!]
	\centering
	\includegraphics[width=0.75\textwidth]{figures/discovery}
	\caption{Application nodes in the same network must become aware of each other}
	\label{fig:discovery}
\end{figure*}

%2: Nowadays Pervasive and mobile computing
\subsection{Pervasive and Mobile Applications}~\label{sec:characterization}

%TODO: fix this paragraph
The market of applications crafted for pervasive and specially mobile devices continues to increase as more people have access to this kind of devices. In specific, applications can be hosted by pervasive devices with less degree of mobility, such as smartphones, tables, gadges, and miniaturized computer platforms like Arduino and Raspberry PI, as well as the wireless networks previously mentioned, 
...
The kind of application targeted by the proposed framework have some or all of the following characteristics:

\begin{enumerate}[label=C\arabic*]
	
	\item \textbf{Connectivity:} devices are expected to communicate with each other, while in the same area, through infrastructure Wi-Fi or mobile ad hoc networks (MANETs);
	...
	\item \textbf{Intensity:} application nodes may be required to perform frequent resource-intensive tasks, exchange large volumes of data, or a mix of both;
	
	\item \textbf{Delay-sensitive:} among the application features, some may require low latency communication;
...
%Each of these characteristics imposes challenges to the engineering of mobile applications for which interactions among nodes is a requirement. Next, we present a case example for which the aforementioned characteristics apply.



%TBA: a music streamming app with an use case that lets people to collaborativelly manage a playlist and send the audio to a nearby bluetooth
