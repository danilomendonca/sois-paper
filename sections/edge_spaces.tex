...
%As a principle, we state that a new solution must avoid disrupting the current mindset for the development of pervasive applications. Accordingly, a new model must seamlessly cope with the paradigm of service-orientation and cloud-computing.
...
To help in the conceptualization and realization of dynamic pervasive ecosystems, we propose the use of the concept of a role as the key abstraction in the specification, design, and programming of what we call \textit{opportunistic organizations}. %and runtime autonomous management 

%TODO: improve the utility of roles in the context of software engineering
...
Rather than defining a new organization model based on the role abstraction, we inherit from the existing works in the literature. Precisely, we extend the definitions of.... %TODO: bring the magic!!!
...
For the applications whose nodes are expected to interact, collaborate and play others than just the role of a client from a backend server, the distinct functionalities of each role form a concern of their own, i.e., they must be designed and programmed accordingly. 
%
